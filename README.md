...
DeepSeek Harness（DSH）的桌宠插件，桌面端和网页端都能用：右下角一只**官方轮廓版小鲸鱼**，随 agent 状态实时切换动画。纯 DOM 实现、零 React 依赖、零运行时第三方依赖，WebAudio 合成音效无音频文件。

<p align="center">
  <img src="docs/demo.gif" alt="桌宠小鲸鱼：连戳到闹脾气 → 甩晕 → 翻肚皮 → 按在底边压扁 → 思考 → 敲代码 → 别的会话在跑 → 全部收工庆祝" width="440">
</p>

<p align="center"><sub>连戳闹脾气 → 甩晕 → 翻肚皮 → 压扁 → 思考 → 敲代码 → 多会话角标 → 全部收工</sub></p>

<p align="center">
  <a href="#在线预览">在线预览</a> ·
  <a href="#安装">安装</a> ·
  <a href="#特性">特性</a> ·
  <a href="#社区-fork">社区 fork</a> ·
  <a href="#开发">开发</a>
</p>
...
预览页 [preview.html](preview.html) 直接加载插件本体 `lib/client.js`，用几个按钮代替 DSH 喂状态；本地用任意静态服务器打开仓库根目录即可。
...
| 表情 | 眼睛跟着情绪变：庆祝时笑成 ^，失落时垂下眼、挑起八字眉还掉眼泪，报错是叉叉眼，睡觉和翻肚皮是弯月眼，闹脾气吊眉，被抓住是 >_< |
...
| 能力 | 说明 |
|---|---|
| 基础交互 | 单击戳戳、双击 360° 翻滚、拖拽时眼睛变成动漫勾勾眼（>_<）、右键菜单（投喂 / 摸摸头 / 换颜色 / 假装工作 / 思考链 / 隐藏 / 定时 / 关闭 / 音效）、鼠标追光、20s 无操作打瞌睡、拖拽移动并记忆位置 |
| 摸头 | 不用按键，鼠标在它头顶来回蹭就是摸头，光标会变成一只小手。蹭第二下它开始眯眼、腮红变深，每蹭一下头顶被按下去一点，蹭满五下冒颗爱心；一直狂蹭（一秒多里来回八下）它会吊起眉毛说「再摸就秃头了」，然后游开，五秒内不给摸。失落时摸它算安慰。右键「摸摸头」和长按 0.7 秒是同一套反应 |
| 连戳升级 | 戳一两下照常撒娇，戳到三五下开始不耐烦、侧身躲开，六下以上就扭头吊眉闹脾气；停手 2.6 秒后重新计数；agent 一开工，脾气马上就收了 |
...
| 贴边压扁 | 拖到屏幕边缘按上去会被压扁：左右两边压窄，上下两边压成鲸鱼饼，从被按住的那条边开始压；按在角落里只按左右算。离开边缘自己弹回来 |
| 拖着不动 | 抓起来悬在半空超过 2 秒，它开始扭腰问你还在不在；你一动它就不问了 |
| 智能避让 | idle 时若光标在身侧停留 0.9s，鲸鱼会自己让开；抓取/拖拽/右键立即取消避让并冷却 8s，绝不抢交互 |

<details>
<summary><b>提醒</b>：完成提醒、久坐提醒、定时隐藏</summary>
...
<details>
<summary><b>外观与设置</b>：换肤、大小、音效、隐藏召回、多语言等</summary>

| 能力 | 说明 |
|---|---|
| 换肤 | 7 套预设色板（默认**主题蓝**）：主题蓝 / 陶土 / 深海蓝 / 抹茶绿 / 樱粉 / 墨灰 / 夜黑；夜黑为深色皮肤示例（眼睛自动反白）。扩展只需在 `src/client/palettes.ts` 加一行 |
| 自定义大小 | 右键 →「外观 → 大小」在小 / 标准 / 大 / 特大之间循环，0.8 到 1.6 倍。缩放会同步影响碰撞边界、水花位置和贴边判定，不只是看着变大 |
...
| 小按钮 | 隐藏时小按钮随 agent 状态变色呼吸（idle 蓝 / think 深蓝 / working 橙 / celebrate 绿 / error 红），可拖拽，位置用 localStorage 记忆 |
...
- [patches/](patches/) 是早期针对 DSH rc.6 写的个人界面补丁，不属于本插件，已不维护，仅留作参考。

### Hot Reload

前提：DSH profile 用 `link:` 方式安装本仓库（不是 GitHub/npm 的静态副本），且 DSH `>=0.1.5-alpha.2`。
