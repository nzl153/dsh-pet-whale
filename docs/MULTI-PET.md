...
`idleDrift: false` 关掉 idle 的"随机小挪动"（默认开启，鲸鱼/猫会自己游走溜达）。
**站着的角色一定要关**：`microSwim` 每 9~17 秒有 35% 概率把她平移最多 ±100px / ±70px，
但那位移没有任何对应动作（不像在飞也不像在走），看着就是"无缘无故到处飘" ——
哪怕「御剑/游泳」是关着的也会发生。想让这种角色动，就引导用户开「御剑」（游泳模式）。
